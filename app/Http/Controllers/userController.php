...
class userController extends Controller
{
    ...
    function chooseAccountRegisterType(Request $req)
    {
        $req->validate([
            'radio' => 'required|in:teacher,student'
        ]);
        if ($req['radio'] == "student") {
            return redirect('registerAccount');
        } else if ($req['radio'] == "teacher") {
            return redirect('teacherRegisterAccount');
        }
    }
    ...
    function getTeacherAccountRegisterInfo(Request $regReq)
    {
        $regReq->validate([
            'username' => 'required',
            'cid' => 'required | max:4',
        ...
        $teacherUsername = $regReq->username;
        $teacherPass = $regReq->teacher_password;
        $teacherEmail = $regReq->email;
        $teacherCourseID = $regReq->cid;
        $teacher = DB::select("SELECT *
                            FROM teachers
                            WHERE cid = ? or username = ? or email = ?", [$teacherCourseID, $teacherUsername, $teacherEmail]);
        $courseTest = NULL;
        $courseTest = DB::select('SELECT cid
                                      FROM courses 
                                      WHERE cid = ?', [$regReq->cid]);
        if ($teacher) {
            $_SESSION['error'] = "Already registered";
            ...
            $encryptedTeacherPass = md5($teacherPass);
            $teacher = array('username' => $regReq->username, 'cid' => $regReq->cid, 'email' => $regReq->email, 'teacher_password' => $encryptedTeacherPass);
